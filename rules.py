...
RULES_BASE = {
  "仕事内容・募集条件":[
    ...
    {"pattern": r"(若手活躍中|若手が活躍|若い人でも活躍)", "weight": 1, "reason": "若手を強調 → 離職率が高い可能性"},
  ...
  "給与・待遇":[
    ...
    {"pattern": r"(歩合|出来高|インセンティブ)(あり|のみ|中心|比率が高い)?", "weight": 1, "reason": "固定給依存度が低い可能性"},
    {"pattern": r"(固定給|基本給)\s*(未記載|不明|なし)", "weight": 3, "reason": "最低保障不明（高リスク）"},
    {"pattern": r"固定残業[^0-9]*(4[0-9]|[5-9][0-9])\s*時間", "weight": 3, "reason": "固定残業40h以上"},
    {"pattern": r"(固定|みなし).{0,6}残業\s*(\d+)\s*時間|みなし.*?(\d+)\s*時間|みなし残業\s*\d+\s*時間\s*含む", "weight": 3, "reason": "固定/みなし残業を内包（含む）"},
  ...
  "勤務時間・休日":[
    ...
    {"pattern": r"(月(平均)?\s*)?(残業|時間外).{0,6}(6[0-9]|7[0-9]|80)\s*時間", "weight": 3, "reason": "長時間残業（60–80h）"},
    {"pattern": r"(早出|深夜|早朝).*(対応|勤務)", "weight": 2, "reason": "不規則勤務の示唆"},
    {"pattern": r"(週休|休日).*(不定|シフト制のみ|応相談のみ)", "weight": 2, "reason": "休日体制が不透明"},
    {"pattern": r"繁忙期.*(土曜|日曜).*(出勤|出社)", "weight": 2, "reason": "休日出勤の示唆"},
  ],
  "勤務地・募集人数":[
  ...
  "社風・福利厚生":[
    {"pattern": r"アットホーム|社員は家族|一体感|体育会系|家族のよう|活気ある職場", "weight": 1, "reason": "精神論でハードワークを正当化の恐れ"},
    ...
    {"pattern": r"やる気さえあれば(誰でも)?成長できる", "weight": 1, "reason": "精神論・属人性強調"},
  ],
  "求人票サイン":[
    {"pattern": r"平均年齢.?20代|20代が中心|20代中心", "weight": 2, "reason": "若年偏重 → 高離職率の可能性"},
...
SAFE_GUARDS = {
  ...
  "勤務時間・休日":[
    ...
    {"pattern": r"完全?週休\s*2日|完全週休二日|年間休日\s*(120|125|130)\s*日", "negative_weight": 1, "note":"休日制度が明確/十分"},
...
# 閾値を引き締め
THRESHOLDS=[(0,5,"低（比較的安全）"),(6,12,"中（注意が必要）"),(13,999,"高（ブラックの可能性大）")]
