...
def preprocess_text(raw: str) -> str:
    ...
    noise_keys = ["最近見た求人","おすすめ求人","会員登録","キーワードで探す",
                  "スカウト","応募履歴","関連の求人","閲覧履歴","人気のキーワード","この求人を保存"]
    for k in noise_keys:
        txt = txt.replace(k, " ")
    txt = re.sub(r"[ \t\u3000]+", " ", txt)
    txt = re.sub(r"\n{3,}", "\n\n", txt)
...
RULES_BASE = {
  ...
  "勤務時間・休日":[
    ...
    {"pattern": r"みなし残業|固定残業|フレックスタイム.*?コアなし", "weight": 2, "reason": "際限ない稼働の懸念"},
  ...
  "社風・福利厚生":[
    {"pattern": r"アットホーム|社員は家族|一体感|体育会系|家族のよう", "weight": 1, "reason": "精神論でハードワーク正当化の懸念"},
  ],
  "求人票デザイン":[   # ← ラベル変更
    {"pattern": r"平均年齢.?20代", "weight": 2, "reason": "平均年齢20代 → 高離職率の可能性"},
  ],
  "企業HPデザイン":[   # ← ラベル変更
    {"pattern": r"(実績|事例|取引先).*(未記載|非公開|なし)", "weight": 2, "reason": "実績が確認できない"},
  ],
}
...
def score_text(text: str, sector: str | None = None):
    ...
    cat_scores = {}; cat_hits = {}; cat_safe_hits = {}; cat_evidence = {}; cat_status={}

    for cat, rules in RULES_BASE.items():
        score = 0; hits = []; safe = []
        for rule in rules:
            if re.search(rule["pattern"], text, flags=re.IGNORECASE|re.DOTALL):
                score += rule["weight"]; hits.append(rule)
        for guard in SAFE_GUARDS.get(cat, []):
            if re.search(guard["pattern"], text, flags=re.IGNORECASE|re.DOTALL):
                score -= guard["negative_weight"]; safe.append(guard)
        score = max(0, min(score, MAX_PER_CATEGORY))
        cat_scores[cat] = score
        cat_hits[cat] = hits
        cat_safe_hits[cat] = safe
        cat_evidence[cat] = []
        cat_status[cat] = ("測定不能" if (score==0 and not hits and not safe) else "評価済み")

    total=sum(cat_scores.values())
    return cat_scores, cat_status, total
